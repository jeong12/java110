...
@WebServlet("/app/*") //여기까지가 servel path. 이 아래가 pathinfo
public class ServerApp implements javax.servlet.Servlet{
    ...
    private void logBeansOfContainer() {
        ...
        public void service(ServletRequest request, ServletResponse response) throws ServletException, IOException {
            ...
            // 이 메서드에서 요청을 처리할 컨트롤러의 메서드를 찾아 호출하면 됨. 
    ...
    public String getServletInfo() {
        // 서블릿 컨테이너 관리자화면에서 이 서블릿의 정보를 출력할 때 이 메서드를 호출
    ...
    @Override
    public void destroy() {
        // 서블릿 컨테이너는, 서버를 종료하거나 웹 애플리케이션을 정지하기 바로 직전에 이 메서드를 호풓
        // 왜?
        // 이 서블릿이 사용했던 자원을 해제시켜 메모리를 줄일 수 있도록 하기 위해 이 메서드를 호출한다.
        // 예를 들어 DB 연결된 것을 닫는다거나, 열린 파일을 닫는다거나, 연결된 소켓을 다든 증의 작업을 이 메서드에서 수행
