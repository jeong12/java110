...
public class AuthServiceImpl implements AuthService {
  ...
  @Override
  public Member getFacebookMember(String accessToken, String type) {

    // Facebook 에 Graph API 실행하기
    ...
    // 원격 서버에서 보낸 JSON 문자열을 Map 객체로 자동 변환하려면 Json 문자열을 처리하는 라이브러리를 추가해 둬야 한다.
    // 따로 코드를 추가할 필요는 없다. => Gson 또는 Jackson 라이브러리

    @SuppressWarnings("rawtypes")
    Map response = restTemplate.getForObject("https://graph.facebook.com/v3.2/me?access_token={v1}&fields={v2}", 
        Map.class,accessToken,"id,name,email"); // 값을 개별적으로 넘기지 않고 맵에 담아 넘길 수 있다.

    // FaceBook 사용자의 이메일로 현재 서버에서 사용자 정보를 찾는다.
    Member member=null;
    ...
    // 해당 회원을 현재 서버에서 찾았으면 그 정보를 리턴하라.
    if(member != null) 
      return member;

    // 현재 서버에 가입하지 않았다면 페이스북 기본 정보를 가지고 자동으로 
  ...
  private Member createMember(String type,String email, String name, String password) {
