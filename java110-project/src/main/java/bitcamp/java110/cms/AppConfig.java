...
// Mybatis 에서 자동으로 DAO를 생성할 때 사용할 인터페이스가 들어있는 패키지 설정 
@MapperScan("bitcamp.java110.cms.dao")
public class AppConfig {
    
    @Autowired
    Environment env;
// jdbc. 붙이는 이유는 위의 환경변수에 덮어쓰는데 그 과정에서 오류가 생겨서(env는 JVM의 정보를 가져옴) 
    ...
    public SqlSessionFactory sqlSessionFactory(DataSource dataSource,ApplicationContext appCtx)  {
        ...
        try {
            //SqlSessionFactory를 FactoryBean이 만듬!
            SqlSessionFactoryBean factory = new SqlSessionFactoryBean();
            
            //DB 커넥션풀을 관리해주는 객체를 꼽는다.
            factory.setDataSource(this.dataSource());
            
            // SQL 맵퍼 파일에서 도메인 객체의 별명을 사용하려면 도메인 객체가 들어있는 패키지를 지정해야 한다.
            // 그러면 Mybatis가 해당 패키지의 모든 클래스에 대해 별명을 자동으로 생성할 것이다.
            factory.setTypeAliasesPackage("bitcamp.java110.cms.domain");
            
            // SQL 맵퍼 파일 경로를 등록한다.
            factory.setMapperLocations(appCtx.getResources("classpath:/bitcamp/java110/cms/mapper/**/*.xml"));
           // /**/의미. 현재 폴더 뿐만 아니라 그 것의 하위 폴더에서도 찾음
