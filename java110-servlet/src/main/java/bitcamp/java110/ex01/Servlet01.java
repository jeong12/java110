...
/* 서블릿을 만들었으면 서블릿 컨테이너에 등록해야 한다.
   ...
   1) 웹 애플리케이션 경로를 root(/)로 설정했을 경우,
      http://localhost:8888/ex01/servlet01 
   2) 웹 애플리케이션 경로를 web01로 설정했을 경우,
 ...
 만약 이미 만든 서블릿에 대해 변경했을 경우, 5초 이내에 자동으로 로딩된다.*/
...
public class Servlet01 implements Servlet{
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        // 예를 들면, DB 커넥션을 준비한다거나 외부 서버에 소켓을 연결하는 등의 작업들.
        // 또는 서블릿에서 사용할 Service 객체나 DAO 객체를 준비하는 일을 한다. 
    ...
    public ServletConfig getServletConfig() {
        // 서블릿을 실행하다가 서블릿 정보가 필요할 때 이 메서드를 호출하여 ServletConfig 객체를 리턴받아 사용한다.
        // 그래서 이 메서드는 init()에서ㅏ 파라미터로 받은 ServletConfig 
        // 잘 보관하였다가 호출되었을 때, 객체를 리턴해야 한다. 
        return this.config;
    }
    ...
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        ...
        // 예를 들면, 게시물 목록을 출력하거나 로그인을 처리하거나 회원가입을 처리하는 등의 일을 한다. 
    ...
    public String getServletInfo() {
        ...
        // 주로 서블릿 컨테이너의 관리 메뉴에서 서블릿 정보를 출력할 때 이메서드의 리턴 값을 받아서 출력한다.
        return "Servlet 01";
    }
...
* - 해당 서블릿의 인스터가 이미 생성되었다면 = 해당 서블릿 객체가 있다면, 
*   - service()를 호출한다.
* - 해당 서블릿의 인스터가 이미 생성되지 않았다면 = 해당 서블릿 객체가 없다면,  
*   - 서블릿 객체 생성 및 생성자 호출
*   - init()호출
*   - service()호출
