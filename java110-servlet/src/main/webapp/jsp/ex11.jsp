...
- 빌트인 객체 (이름을 반드시 지켜야함!)(9개)
    request:HTTPServletRequest      - _jspService()메서드의 파라미터
    response:HTTPServletResponse    - _jspService()메서드의 파라미터
    pageContext:PageContext         - _jspService()메서드의 로컬 변수
    session:HttpSession             - _jspService()메서드의 로컬 변수
    application:ServletContext      - _jspService()메서드의 로컬 변수
    config:ServletConfig            - _jspService()메서드의 로컬 변수
    out:JspWriter                   - _jspService()메서드의 로컬 변수
    page:Object                     - _jspService()메서드의 로컬 변수
        -현재 서블릿 객체를 가르킨다. 즉 this이다.
    exception:Throwable             - _jspService()메서드의 로컬 변수
...
// 위 9개의 변수는 JSP 페이지에 존재하는 변수이기 떄문에 컴파일 오류가 발생하지 않는다.
// 그러나 다음 "ok"와 같은 임의의 변수는 존재하지 않기 떄문에 컴파일 오류가 발생한다.
//out.println(ok);
%>
<hr>
</body>
