...
public class Test23 {
    ...
    public String m1(Model model) throws Exception{
        ...
        /* 다음과 같이 JSP 파일이 웹 브라우저에서 요청할 수 있는 폴더에 두게 되면,
         * 페이지 컨트롤러를 경유하지 않고 실행될 수 있기 때문에 
         * (물론 아무런 의미가 없겠지만) 이것을 방지하기 위해 보통 JSP 파일을
         * WEB-INF 폴더 아래에 둔다.
         * 
         * return "ex/03/Test23.jsp";
         */
        
        /* 믄제는 WEB-INF 폴더 아래에 JSP파일을 두게 되면, 
         * 리턴 URL을 작성하는데 너무 길어서 번거롭다.
         *
            return "/WEB-INF/jsp/ex03/Test23.jsp";

         * 이를 해결하기 위해 InternalResourceVicewResolver를 사용한다.
         * 공통 경로는 InternalResourceVicewResolver에 등록해 두고,
         * reuqest handler에서는 공통 경로는 제외한 나머지 경로만 지정하기 떄문에 편리하다.
         * 
         */
    ...
    public void m2(Model model) throws Exception{
         ...
         * 즉 request handler가 JSP 값을 리턴하지 않으면, request handler에 지정된 url로
         * JSP를 찾는다.
         * 
