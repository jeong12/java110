...
@RestController //= @controller + @ResponseBody <-- google gson lib가 있어야 함. 
public class Test22 {

    /* 페이지 컨트롤러에 @RestController를 붙이면 
     * 프론트 컨트롤러는 request handler의 리턴 값을 클라이언트로 바로 출력한다.
     * 1) 리턴 타입이 String, primitive type 일 경우, => 문자열로 변환한 후 출력한다.
     * 2) 리턴 타입이 기타 객체일 경우, 
     *    => HttpMessageConverter 규칙에 따라 만든 클래스를 찾아 그 클래스를 이용하여 
     *       리턴한 객체의 값을 JSON 형식의 문자열로 변환한 후 출력한다
     * 실행 원리:
     * 1) HttpMessageConverter 규칙에 따라 만든 클래스를 찾는다.
     *    => "HttpMessage"는 클라이언트로부터 받은 데이터, 클라이언트로 보내는 데이터 를 가리킨다.
     * 2) 만약 HttpMessageConverter 구현체를 못 찾았다면?
     *    => 리턴 값을 바꿀 수 없기 때문에 실행 오류가 발생한다! 
     *
     *    HttpMessageConvert 구현페는 어떻게 추가하는가?
     ...
     * 프론트 컨트롤러는 리턴 객체를 Json 변환기를 이용하여 변환한 후 클라이언트로 출력한다.
     */
    ...
    //프론트 컨트롤러는 리턴 값을 Json으로 인식하지 않고 문자열로 인식하고 그대로 출력한다.
     ...
     * 프론트 컨트롤러는 request handler의 리턴 타입에 따라 어떻게 처리할 지 결정한다.
     * String 이라면 그대로 출력하고, 다른 타입이라면 JSON data로 변경하여 출력한다.
     * 
     */
    @RequestMapping("m3")
  ...
  //@RestController 일 경우 리턴 타입이 primitive type 이라면 "문자열로 변환하여 출력한다."
    ...
    // 생략하면 클라이언트가 보낸 데이터가 
    // 일반, application/x-www-form-urlencoded형식의 데이터로 간주한다.
    // 즉, 변수=값 & 변수=값& .. 형식의 데이터로 간주하고 Car 객체에 값을 넣으려고 할 것이다.
    // 그러나 클라이언트는 JSON 형식으로 데이터를 보내기 때문에 Car 객체에 잘 들어가지 않는다.
    @RequestMapping("m8")
     ...
     * 단, MAP 변수를 그냥 선언하면 기존의 규칙대로 프론트 컨트롤러는 페이지 컨트롤러가 작업한
     * 결과를 담는 용도로 사용하는 줄 알고 빈 맵 객체를 넘겨줄 것이다.
