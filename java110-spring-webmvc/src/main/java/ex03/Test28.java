// 세션 다루기 - @ModelAttributes annotation 과 Session Status
package ex03;
...
public class Test28 {
    ...
    public String m1(Model model,HttpSession session) throws Exception{
      ...
      //=> @SessionAttributes에 지정된 이름이기 때문이다.
        ...
        model.addAttribute("tel2", "2222-2222");
              
        // 프론트 컨트롤러가 ServletRequest에 값 보관. 페이지 컨트롤러가 직접 세션에 보관
        // =>@SessionAttributes에 지정된 이름이 아니기 떄문이다.
        model.addAttribute("gender2", "woman");
    ...
    @ResponseBody
    public String m4(
            /* 다른 페이지 컨트롤러의 @SessionAttributes에 지정된 값을 받을 때?
    ...
    public String m5(HttpSession session) throws Exception{
       /* 다른 페이지 컨트롤러의 @SessionAttribute에서 지정한 값을 HttpSession 객체에서 직접 꺼낼 때?
        * => 꺼낼 수 있다. 
        */
    ...
    public String logout2(SessionStatus status) throws Exception{
        ...
        return "@SessionAttribute 관리 대상을 제거";
    }
 ...
 * 
 * 2)값 제거
 *   HttpSession.invalidate()는 세션을 오나전히 무효화 시킨다.
 *   SessionStatus.setComplete()은 해당 페이지 컨트롤러의 @SessionAttributes에 지정된 
 *   값만 세션 객체에서 제거한다. 
