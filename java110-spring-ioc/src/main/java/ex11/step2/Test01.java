...
public class Test01 {
    ...
    public static void main(String[] args) {
        ...
        /* 만약 Service클래스의 add()메서드 호출 앞/뒤로 뭔가 다른 일을 수행하고 싶다면?
         * 1) 기존 클래스를 손대지 말야아 하는 경우
         *    즉, 이미 다른 프로젝트에서 기존 클래스를 사용하고 있을 떄!
         *      
         * 2) 기존 클래스의 소스 코드가 업어서 손댈 수 없는 상황이라면?
         *    즉, 직접 만든 클래스가 아니라 외부 라이브러리에 들어있는 클래스를 사용할 떄!
         *    
         * => 프록시 패턴을 적용한다.
         * => 즉 기존 클래스와 똑같은 규칙으로 클래스는 만든다.
         *    그리고 그 클래스를 기존 클래스처럼 사용한다.
         *    이 클래스를 "프록시(proxy)" 라고 부른다.
         * => 프록시 클래스를 정의했으면 기존 클래스를 사용하는 대신에
         *    프록시 객체를 만들어 사용한다.
         *    다만, 프록시를 호출하면 프록시는 기존 클래스의 메서드를 호출하는 방식으로 동작한다.   
         */
        
        // IoC 컨테이너가 리턴하는 것은 ServiceProxy 클래스이다.
